...
void raise_box(void){						// raises rack and pinion
  ...
  delay(5000);								// waits 2secs (ADJUST THIS TO GIVE FULL MOTION)
  rlink.command(MOTOR_3_GO, 0);
}

void lower_box(void){						// lowers rack and pinion
  rlink.command(MOTOR_3_GO, 255); 			// sets motor to down
  delay(5000);								// waits 2secs (ADJUST THIS TO GIVE FULL MOTION)
  rlink.command(MOTOR_3_GO, 0);				// Stops motor
} 

void extend_actuators(void){
	pic_current = rlink.request(READ_PORT_3);
	param = pic_current bitor bit_raise;	// ORs current bits with 0100 0000 ie sets bit7 high and leaves rest as they are
	rlink.command (WRITE_PORT_3, param);
}

void retract_actuators(void){
	pic_current = rlink.request(READ_PORT_3);
	param = pic_current bitand bit_lower;	// ANDs current bits with 1011 1111 ie sets bit7 low and leaves rest as they are
	rlink.command (WRITE_PORT_3, param);
}
